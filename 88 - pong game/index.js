...
function checkCollision() {
    ...
    if (ballX <= (paddle1.x + paddle1.width + ballRadius)) {
        if (ballY > paddle1.y && ballY < paddle1.y + paddle1.height) {
            ballX = (paddle1.x + paddle1.width) + ballRadius // if ball gets stuck inside the paddle
            ballXDirection *= -1
            ballSpeed += 1
        }
    }
    if (ballX >= (paddle2.x - ballRadius)) {
        if (ballY > paddle2.y && ballY < paddle2.y + paddle2.height) {
            ballX = paddle2.x - ballRadius // if ball gets stuck inside the paddle
            ballXDirection *= -1
            ballSpeed += 1
        }
    }
...
function changeDirection(event) {
    ...
    const paddle1Up = 65 // a
    const paddle1Down = 90 // z
    const paddle2Up = 38 // ArrowUp 
    const paddle2Down = 40 // ArrowDown
